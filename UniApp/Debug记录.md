...
7. 有时 `@tap` 事件会被上面覆盖的元素挡住（疑似只在模拟器触发，真机没有）
8. map 组件中，点击地图空白处的事件是`@tap`，点击“地名”时触发`@poitap`，两者只会触发一个
9. $refs.xxx 调用出现问题（访问 null 的 metadata）可能是因为跨分包引用组件，应该将组件放入主包
10. scroll-view 难以控制“自定义滚动逻辑”，因为它有无法关闭的“惯性”，可以考虑改为普通view+内部内容transform:translateY+touch监听，实现自定义滚动效果 