...
输出：
- a = ________
    - 100 **❌有误，正确答案：1**（p1 后来被赋值为 p2，指向 b，修改的是 b）
- b = ________
    - 100
...
```
- 返回局部变量的指针（逃逸），不推荐这么做 **❌不完整，正确答案：代码可以正常运行，Go 编译器会进行逃逸分析，将 x 分配到堆上。但这不是好的编程习惯，应该避免返回局部变量指针，改用传入指针参数或返回值的写法**
